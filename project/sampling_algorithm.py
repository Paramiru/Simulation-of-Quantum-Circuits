...
class SamplingAlgorithm():
    ...
    def sample_random_circuit(self, fc, N):
        if fc % 2 != 0:
            raise ValueError("Invalid Fourier Coefficients")
        elif N > np.log2(len(fc)):
        ...
        for _ in range(N):
            ...
            prob_add_zero = self.__get_prob_add_zero(prob_limit, outcome)
        return outcome
    
    def sample_events(self, N: int, k: int):
        ...
        for _ in range(N):
            self.sample_random_circuit(self.fourier_coeff)
